...
export async function deleteColumn(app, noteUUID, columnId) {
  ...
  // Extract content of the column being deleted (trimmed of leading/trailing blanks)
  const extracted = lines.slice(span.contentStart, span.contentEnd)
    .filter((line, i, arr) => !(line.trim() === "" && (i === 0 || i === arr.length - 1)));

  // Target column to absorb tasks: prefer previous header (colIdx - 1), fallback to next header (colIdx + 1)
  const targetCol = colIdx > 0 ? columns[colIdx - 1] : columns[colIdx + 1];

  // Remove the deleted column's heading + content
  const next = [
  ...
  if (extracted.length > 0 && targetCol) {
    if (colIdx > 0) {
      // Deleting a column after the first: append to end of previous column's content
      const insertAt = targetCol.contentEnd;
      next.splice(insertAt, 0, ...extracted);
    } else {
      // Deleting the first column: insert at start of next column's content
 ...
 * during rapid clicks or concurrent operations.
 */
