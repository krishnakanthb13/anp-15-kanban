...
export function buildColumnSpans(markdown, columnLevel) {
  ...
  const columnHeadings = (columnLevel !== undefined && columnLevel !== null)
    ? headings.filter(h => h.level === columnLevel)
    : headings;
...
export function findTaskLines(lines, tasks) {
  ...
  for (const task of tasks) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // STRICT requirement: the line MUST be an actual markdown task checkbox line
...
/**
 * Computes subtaskDepth, isSubtask, and isParent for tasks based on markdown indentation
 * and Amplenote hierarchy metadata.
 *
 * @param {string[]} lines - Note markdown lines
 * @param {Array<Object>} tasks - Tasks in note
 * @param {Map<string, number>} [taskLines] - Line index lookup map
 */
export function detectTaskHierarchy(lines, tasks, taskLines) {
  if (!tasks || tasks.length === 0) return;
  const lookup = taskLines || (lines ? findTaskLines(lines, tasks) : new Map());

  for (let i = 0; i < tasks.length; i++) {
    const task = tasks[i];
    const lineIndex = lookup.get(task.uuid || task.id);
    if (lineIndex !== undefined && lineIndex >= 0 && lines && lines[lineIndex] !== undefined) {
  ...
  // Detect parent task status based on depth progression
  for (let i = 0; i < tasks.length; i++) {
...
export function insertUnderHeading(lines, span, taskLine) {
  ...
  // If the line immediately following is regular non-task text,
  // add a blank line after the task so Amplenote's parser never merges the text into the task content.
  if (
    nextLine !== undefined &&
    nextLine.trim() !== "" &&
...
/**
 * Finds a column span by line ID or heading name.
 * @param {Array<{id: string, name: string}>} spans - Column spans.
 * @param {string} columnId - Positional column id (heading line index) or explicit col_ prefix.
...
export function resolveSpan(spans, columnId, columnName) {
  ...
  // Explicit index prefix match only (e.g., "col_0", "idx_1") to prevent line-number collisions
