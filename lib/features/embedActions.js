...
import { createTaggedNote, openNote, openTag, swapNoteTag } from "../api/noteOps.js";
import { buildTagsBoard } from "../api/tagsBoard.js";
...
// In-flight locks to prevent duplicate board/note creations during rapid concurrent clicks
let inFlightAddTab = false;
let inFlightCreateColumnNote = false;
...
export async function handleMoveCard(app, payload) {
  ...
  if (tab.kind === "tags") {
    ...
    return {
      ...
      tabId: tab.id,
      board,
      toast: changed ? `Note moved to #${toTag}` : undefined,
    };
  }
...
export async function handleAddTagColumn(app, payload) {
  const tab = await resolveNoteTab(app, payload);
...
export async function handleRemoveTagColumn(app, payload) {
  const tab = await resolveNoteTab(app, payload);
  if (!tab || tab.kind !== "tags") return { ok: false };
...
export async function handleCreateNoteInTagColumn(app, payload) {
  const tab = await resolveNoteTab(app, payload);
  if (!tab || tab.kind !== "tags") return { ok: false };
...
export async function handleReorderTagColumns(app, payload) {
  const tab = await resolveNoteTab(app, payload);
