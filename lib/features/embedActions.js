...
export async function handleCardMenu(app, payload) {
  ...
  if (choice === "link_note" || choice === "label") {
    ...
    // Safely escape the note name for the regex
    const escapedName = noteName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

    // 1. Convert any standalone brackets into a markdown link (ignoring ones already formatted as a link)
    const bracketRegex = new RegExp(`(?:\\\\?\\[)+\\s*${escapedName}\\s*(?:\\\\?\\])+(?!\\s*\\()`, "gi");
    let newContent = currentContent.replace(bracketRegex, noteLink);

    // 2. Fix the (url)(url) duplication glitch from your previous tests
    ...
    // 3. REMOVE DUPLICATE ADJACENT LINKS: e.g., [Note](url) [Note](url) -> [Note](url)
    ...
    // 4. If the link STILL isn't in the task at all, append it to the end
    if (!newContent.includes(noteUUID)) {
      newContent = newContent ? `${newContent} ${noteLink}` : noteLink;
    }

    // 5. Clean up any stray double spaces left behind by replacements
    newContent = newContent.replace(/\s{2,}/g, " ").trim();

    // Update the task only if the content actually changed
