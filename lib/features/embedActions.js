...
export async function handleCreateCard(app, payload) {
  ...
  if (tab.kind === "tag" || tab.kind === "notes") {
    ...
    if (taskUuid && board && Array.isArray(board.columns)) {
      const alreadyInBoard = board.columns.some(col => (col.cards || []).some(c => c.id === taskUuid || c.uuid === taskUuid) || (col.sections || []).some(sec => (sec.cards || []).some(c => c.id === taskUuid || c.uuid === taskUuid)));
      if (!alreadyInBoard) {
