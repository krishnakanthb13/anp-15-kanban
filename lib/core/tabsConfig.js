...
export function normalizeConfig(raw) {
  ...
  const tabs = Array.isArray(raw.tabs)
    ? raw.tabs.filter(isValidTab).map(t => {
        const cleanTab = { ...t, columnLimits: normalizeColumnLimits(t.columnLimits) };
        if (t.kind === "tags") {
          cleanTab.tags = Array.isArray(t.tags)
            ? t.tags.map(s => typeof s === "string" ? s.trim().replace(/^#/, "") : "").filter(Boolean)
            : [];
        }
        return cleanTab;
      })
 ...
 * @param {{kind: "note"|"tag"|"notes"|"tags", name: string, noteUUID?: string|null, tag?: string|null, tags?: string[]}} spec
 * @returns {{id: string, kind: string, name: string, noteUUID: string|null, tag: string|null, tags?: string[]}}
 */
export function createTab({ kind, name, noteUUID = null, tag = null, tags = [] }) {
  ...
  const cleanTags = Array.isArray(tags)
    ? tags.map(s => typeof s === "string" ? s.trim().replace(/^#/, "") : "").filter(Boolean)
    : [];
